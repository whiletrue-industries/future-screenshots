...
9. **Showcase WS** (`/showcase-ws`)
   ...
   - **Demo mode** (`DemoModeService`) – an unattended camera tour of the canvas, for wall displays. Items never move; the camera goes to them:
     ...
     3. The item is drawn on the overlay canvas, in front of everything, from the moment the flight starts, but the map stays fully visible during the flight. Only on arrival is the main canvas blurred (20px) and faded to 10% (`ThreeRendererService.setDemoDimming()`), and `DemoFocusOverlayComponent` hangs the item from its string with prefer/prevent clip(s) and its favorability and plausibility labels (human evaluation, falling back to the AI's). The labels have no background: the preference is bold and uppercase in the string's colour, an "-ish" suffix stays lowercase in the other side's colour, and the plausibility sits tight beneath it, just above the string. The string wipes in first, then the clips drop in on top of it together with the labels
